class TicTacToe:
    ...
    def mark(self, i, j): 
        ...
        if not (0 <= i <= 2 and 0 <= j <= 2):               # check invalid position
            raise ValueError("Invalid board position.")

        if self._board[i][j] != ' ':                       # check occupied position
            raise ValueError("Board position occupied.")

        if self.winner() is not None:                      # check if game is finished
            raise ValueError("Game is already complete.")

        self._board[i][j] = self._player                   # make the move
        if self._player == 'X':                            # switch player's turn
            self._player = 'O'
        else:
            self._player = 'X'
